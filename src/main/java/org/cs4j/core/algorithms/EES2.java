...
public class EES2 implements SearchAlgorithm {
    ...
    // openF is implemented as a binary heap and actually contains nodes ordered by their f values
    ...
    private void _initDataStructures(SearchDomain domain) {
        ...
        this.gequeue =new GEQueue<>(this.openComparator, new GENodeComparator(), new FocalNodeComparator(), EES2.FOCAL_ID);
        this.openF = new BinHeapF<>(EES2.CLEANUP_ID, this.domain);
     ...
     */
    public EES2(double weight, boolean reopen) {
        this.weight = weight;
        this.reopen = reopen;
        this.zval = 0;//like regular EES
...
/*        if(this.FR==Integer.MAX_VALUE) return "EES";
        else return "EES"+this.FR;*/
        return "EES";
    }
    ...
    public void setAdditionalParameter(String parameterName, String value) {
        switch (parameterName) {
            ...
            case "zval": {
                this.zval =Double.parseDouble(value);
                break;
            }
            case "FR": {
                ...
                for (int i = 0; i < numOps; ++i) {
                    ...
                    if (this.closed.containsKey(childNode.packed)) {
                        ...
                        if (dupChildNode.f > childNode.f) {
                            ...
                            if (dupChildNode.getIndex(EES2.CLEANUP_ID) != -1) {
                                ++result.opupdated;
                                this.gequeue.remove(dupChildNode);
                                this.openF.remove(dupChildNode);
...
//            throw new OutOfMemoryError();
    ...
    /**
     * This comparator is used in order to sort the openF list on F
     */
...
/*    protected final class NodeComparator implements Comparator<Node> {
    ...
    private final class GENodeComparator implements Comparator<Node> {
        @Override
        public int compare(final Node a, final Node b) {
            if (a.fHat < EES2.this.weight * b.fHat) {
                return -1;
            } else if (a.fHat > EES2.this.weight * b.fHat) {
                return 1;
            }
            return 0;
        }
    ...
    private class Node extends SearchQueueElementImpl implements RBTreeElement<Node, Node>, Comparable<Node> {
        ...
        private double hHat;
        private double dHat;
        private double sseH2;
        private double sseD2;
        private double hVar;
        private double dVar;
        ...
        /**
         * @return variance of H
         *
         *
         */
        private double _computeHVar() {
            double hVar = 0;
            if (this.depth > 1) {//for 0 or 1 sample dVar=0;
                hVar = sseH2 - (sseH*sseH/this.depth);
                hVar = hVar/(this.depth-1);//calculate like STDV with n-1 df
            }
            return hVar;
        }
        ...
        /**
         *
         * @return variance of d
         */
        private double _computeDVar() {
            double dVar = 0;
            if (this.depth > 1) {//for 0 or 1 sample dVar=0;
                dVar = sseD2 - (sseD*sseD/this.depth);
                dVar = dVar/(this.depth-1);//calculate like STDV with n-1 df
            }
            return dVar;
        }
        ...
        public void _computePathHats(Node parent, double edgeCost) {
            if (parent != null) {
                ...
                double Eh = ((edgeCost + this.h) - parent.h);
                double Ed = ((1 + this.d) - parent.d);
                this.sseH = parent.sseH + Eh;
                this.sseD = parent.sseD + Ed;
                this.sseH2 = parent.sseH2 + Eh*Eh;
                this.sseD2 = parent.sseD2 + Ed*Ed;
            }

            this.hHat = this._computeHHat();
            this.dHat = this._computeDHat();
            this.hVar = this._computeHVar();
            this.dVar = this._computeDVar();
            this.fHat = this.g + this.hHat + zval * this.hVar;
